...
## Restructure the repository into a frontend/backend monorepo

`Title`: Reorganise the repo into `pomodoro-frontend/` and `pomodoro-backend/`

`User prompt`: I want to restructure this project into a monorepo while preserving the current Claude Code conversation and context. The project currently contains only the frontend. Please reorganise it so the repository root contains: `pomodoro-frontend/` — move the entire existing frontend project into this directory; `pomodoro-backend/` — create this as an empty directory for the backend. Before making any changes, analyse the current project structure and determine everything that belongs to the frontend. Move only the necessary files and folders, preserving all functionality. Update any paths, scripts, configuration files, workspace settings, or references that are affected by the move. Do not refactor or modify application logic unless required because of the new directory structure. After restructuring, verify that the frontend still installs, builds, and runs correctly from the `pomodoro-frontend` directory. Once the restructuring is complete, remain in the same Claude Code conversation and treat the repository in its new layout as the current project. Re-analyse the entire repository so your understanding is updated to include both `pomodoro-frontend` and `pomodoro-backend`. Consider this new structure as the baseline for all future discussions and implementation.

## Separate frontend and backend configuration for independent development

`Title`: Split `.gitignore` per package and reorganise `.claude` for a monorepo

`User prompt`: Now that the project has been separated into `pomodoro-frontend` and `pomodoro-backend`, I want you to reorganise the repository according to best practices for independent frontend and backend development. The goal is to create a clean structure where frontend and backend can have their own development workflows, configurations, and eventually independent Git repositories. Create separate `.gitignore` files for `pomodoro-frontend` and `pomodoro-backend`. Each `.gitignore` should contain only rules relevant to that specific project. Remove unnecessary duplication. If a root-level `.gitignore` is needed, only keep rules that genuinely apply to the entire repository. Next, review the existing `.claude` directory and all its contents, including rules, agents, skills, commands, prompts, and configuration files. Analyse every `.claude` file and classify it as Shared, Frontend-specific, or Backend-specific. Do NOT create duplicate `.claude` directories at multiple levels. Choose only one of these approaches: a single shared `.claude` directory at the repository root containing all common configurations and properly organised frontend/backend-specific rules, OR separate `.claude` directories inside each project. Prefer the simplest and most maintainable structure. Some existing `.claude` files may have been created while working on the frontend but may still contain instructions useful for backend development — do not assume their purpose from their history; analyse their actual content and decide the best placement. Also review any other configuration files, scripts, or settings that should be separated between frontend and backend to support clean independent development. Do not modify application logic. Focus only on repository structure, configuration organisation, and maintainability.
